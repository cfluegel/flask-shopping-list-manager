...
class App {
  ...
  static async checkPrinterStatus() {
    ...
    try {
      const res = await apiClient._request('GET', '/printer/status');
      const btn = document.getElementById('printer-btn');
      const dot = document.getElementById('printer-dot');
      if (res.success && res.data.available) {
        btn.style.display = '';
        dot.className = 'pwa-printer-dot ' + (res.data.reachable ? 'online' : 'offline');
      } else {
        btn.style.display = 'none';
      }
    } catch (e) {
      // Printer not available or network error — keep button hidden
    }
  }
